...
import { ErrorState, Head, LoadingState, Markdown, PageHeading } from "@app/primitives";
...
const frameworks: { [id: string]: FrameworkInstructions } = {
  tauri: {
    name: "Tauri",
    readme: "https://raw.githubusercontent.com/aptabase/tauri-plugin-aptabase/main/README.md",
    repository: "https://github.com/aptabase/tauri-plugin-aptabase",
  },
  flutter: {
    name: "Flutter",
    readme: "https://raw.githubusercontent.com/aptabase/aptabase_flutter/main/README.md",
    repository: "https://github.com/aptabase/aptabase_flutter",
  },
  webapp: {
    name: "Web App",
    readme: "https://raw.githubusercontent.com/aptabase/aptabase-js/main/README.md",
    repository: "https://github.com/aptabase/aptabase-js",
  },
  swift: {
    name: "Swift (Apple)",
    readme: "https://raw.githubusercontent.com/aptabase/aptabase-swift/main/README.md",
    repository: "https://github.com/aptabase/aptabase-swift",
  },
  maui: {
    name: ".NET MAUI",
    readme: "https://raw.githubusercontent.com/aptabase/aptabase-maui/main/README.md",
...
export function Component() {
  ...
  const { isLoading, isError, data } = useQuery(["markdown", selected], () => fetchInstructions(selected));
    ...
    <>
      ...
      <div className="flex flex-col space-y-8 mt-8">
        <div className="px-4 py-2 bg-primary-50 max-w-fit rounded border border-primary-100">
          <p className="text-secondary text-sm mb-1">👇 This is the app key for {app.name}</p>
          <span className="font-medium text-xl mb-2">{app.appKey} </span>
          <p className="text-secondary mt-2">It's used by the SDK to identify this app.</p>
        </div>

        <div className="flex items-center border-b pb-4 border-gray-200 justify-between">
          <div className="flex items-center space-x-4">
            <span className="whitespace-nowrap">Which framework are you using?</span>
            <select name="framework" defaultValue={selected} onChange={handleChange} className="form-select max-w-fit">
          ...
          {repository && (
            <a
              target="_blank"
              className="hidden md:flex bg-gray-50 hover:bg-gray-100 text-sm border border-gray-200 rounded p-2 items-center space-x-1"
              href={repository}
            >
        ...
        {isLoading ? (
          ...
          <div>
            <Markdown content={(content || "").replace("<YOUR_APP_KEY>", app.appKey)} />
          </div>
        )}
      </div>
    </>
